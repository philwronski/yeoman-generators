...
function askForDefaultAlias() {
  ...
  this.prompt(prompts).then(function(props) {
    ...
    this.gitConfig.alias.push({
      alias: "hist",
      command: "log --pretty=format:'%h %ad | %s%d [%an]' --graph --date=short"
    });
    this.gitConfig.alias.push({
      alias: "type",
      command: "cat-file –t"
...
function askForRemote() {
  ...
  this.prompt(prompts).then(function(props) {
    if (props.add_remote) {
      this.gitConfig.remotes.push({
        "name": props.remote_shortName,
        "value": props.remote_url
      });
      if (props.remote_storeCredential) {
        this.gitConfig.credentials.push({
          "url": props.remote_url,
          "helper": "wincred",
          "username": props.remote_credential_username
        });
      }
      askForRemote.call(this, done);
    } else {
...
function askForFetchRemote() {
  ...
  var prompts = [{
    ...
    {
      when: function(response) {
        return response.fetch_remote;
      },
      type: 'list',
      name: 'selected_remote',
      message: 'Which remote you want to fetch?',
      choices: this.gitConfig.remotes,
      default: this.gitConfig.remotes[0].name,
      store: true
    }
  ];

  if (this.gitConfig.remotes.length > 0) {
    this.prompt(prompts).then(function(props) {
      // To access props later use this.props.someAnswer;
